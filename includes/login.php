...
if (isset($_POST['login']) && !empty($_POST['login'])) {
  ...
  if (!empty($email) or !empty($password)) {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
      ...
      if($getFromU->userBanded($email) === true){
        $errorMsg = "Your account is banded by admin!";
      } else if ($getFromU->login($email, $password) === false) {
...
?>
  <form method="post" autocomplete="off" onsubmit="showLoading()">
    <h1 class="mb-4 " style="text-align:center;">Log in to Hippo Social</h1>
