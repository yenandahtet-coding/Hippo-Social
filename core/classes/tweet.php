...
class Tweet extends User
{
		...
		foreach ($tweets as $tweet) {
			...
			echo '<div class="all-tweet">
                  <div class="t-show-wrap">
                   ...
                   ' . ((isset($retweet['retweetID']) && ($retweet['retweetID'] === $tweet->retweetID || $tweet->retweetID > 0)) ? '
					...
					'<!--tweet show head end-->
			      	...
			      	<div class="t-show-footer">
			      		<div class="t-s-f-right">
			      			<ul>
			      				<li><button style="outline:none;"><i class="fa fa-comment" aria-hidden="true" style="color:brown;"></i></button></li>
			      				<li>' . ((isset($retweet['retweetID']) ? $tweet->tweetID === $retweet['retweetID'] : '') ?
				'<button class="retweeted" data-tweet="' . $tweet->tweetID . '" data-user="' . $tweet->tweetBy . '" style="outline:none;"><i class="fa fa-retweet" aria-hidden="true" style="outline:none; color:brown;"></i><span class="retweetsCount">' . (($tweet->retweetCount > 0) ? $tweet->retweetCount : '') . '</span></button>' :
				'<button class="retweet" data-tweet="' . $tweet->tweetID . '" data-user="' . $tweet->tweetBy . '" style="outline:none;"><i class="fa fa-retweet" aria-hidden="true" style="color:brown;"></i><span class="retweetsCount"style="color:brown;">' . (($tweet->retweetCount > 0) ? $tweet->retweetCount : '') . '</span></button>') . '
			      				</li>
			      				<li>' . ((isset($likes['likeOn']) ? $likes['likeOn'] === $tweet->tweetID : '') ?
				'<button class="unlike-btn" data-tweet="' . $tweet->tweetID . '" data-user="' . $tweet->tweetBy . '" style="outline:none;"><i class="fa fa-heart" aria-hidden="true" style="color:brown;"></i><span class="likesCounter">' . (($tweet->likesCount > 0) ? $tweet->likesCount : '') . '</span></button>' :
				'<button class="like-btn" data-tweet="' . $tweet->tweetID . '" data-user="' . $tweet->tweetBy . '" style="outline:none;"><i class="fa fa-heart-o" aria-hidden="true" style="color:brown;"></i><span class="likesCounter">' . (($tweet->likesCount > 0) ? $tweet->likesCount : '') . '</span></button>') . '
			      				</li>
			               
			                ' . (($tweet->tweetBy === $user_id) ? '
			              	    <li>
			      					<a href="#" class="more"><i class="fa fa-ellipsis-h" aria-hidden="true" style="outline:none; color:brown;"></i></a>
			      					<ul>
	...
	{
		$stmt = $this->pdo->prepare("SELECT COUNT(*) AS postCount FROM tweets WHERE retweetID = 0");
		$stmt->execute();
		$result = $stmt->fetch(PDO::FETCH_OBJ);
		return $result->postCount;
	}

	public function countReposts()
	{
		$stmt = $this->pdo->prepare("SELECT COUNT(*) AS repostCount FROM tweets WHERE retweetID != 0");
		$stmt->execute();
		$result = $stmt->fetch(PDO::FETCH_OBJ);
